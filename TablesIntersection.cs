...
{
    public partial class TablesIntersection : Form
    {
        public Table Result { get; set; }
        private readonly DataBaseManager _dataBaseManager = DataBaseManager.Instance;
        public TablesIntersection()
        {
            InitializeComponent();
            var tablesNames = _dataBaseManager.Database.Tables.Select(i => i.Name).ToArray();
        ...
        private void SearchIntersection(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(firstTableComboBox.SelectedItem.ToString()) || string.IsNullOrEmpty(secondTableComboBox.SelectedItem.ToString()))
                return;
            var res = _dataBaseManager.Intersection(_dataBaseManager.Database.Tables.FirstOrDefault(i => i.Name == firstTableComboBox.SelectedItem.ToString()),
                (_dataBaseManager.Database.Tables.FirstOrDefault(i => i.Name == secondTableComboBox.SelectedItem.ToString())));
            RenderTable(res);
        }
